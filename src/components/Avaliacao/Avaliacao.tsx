...
export default function Avaliacao() {
  ...
  const next = () => {
    setCurrentIndex((prevIndex) => {
      const lastIndex = isMobileView ? lista.length - 1 : lista.length/2 - 1;
  ...
  return (
    <section className="flex flex-col items-center mb-32">
      <div className="flex justify-center w-full">
        <h1 className="mx-4 text-2xl font-bold mt-6 mb-10 text-azulclaro min-[460px]:text-4xl md:text-5xl lg:text-6xl lg:mt-10">
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {(isMobileView ? lista.concat(lista) : lista).map((avaliacao, index) => (
            <div
              key={`${avaliacao.idAvaliacao}-${index}`}
